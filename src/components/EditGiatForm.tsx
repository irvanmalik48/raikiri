...
import { Giat, Lokasi } from "@prisma/client";
...
export default function AddAnggotaForm({ cuid }: { cuid: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/v1/giat?cuid=${cuid}&includeLokasi`)
      .then((res) => res.json())
      .then((data) => {
        setNama(data.name);
        setLokasi(data.lokasi.name);
        setWaktu(new Date(data.waktu));
      });
  }, [cuid]);
      ...
      <button
        ...
        onClick={async (e) => {
          ...
          await fetch("/api/v1/giat/edit", {
            ...
            .then((data) => {
              if (data.error) {
                alert("Gagal mengubah giat! Server error!");
                return;
              }

              alert("Berhasil mengubah giat!");
            });
